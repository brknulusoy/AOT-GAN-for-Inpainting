import importlib
import matplotlib.pyplot as plt

import torch
from torchvision.transforms import ToTensor

from utils.option import args
from data.dataset import TerrainDataset


def demo(args):
    dataset = TerrainDataset(
        args.dir_train,
        dataset_type="train",
        randomize=True,
        block_variance=1,
    )

    # Model and version
    net = importlib.import_module("model." + args.model)
    model = net.InpaintGenerator(args)
    model.load_state_dict(torch.load(args.pre_train, map_location="cpu"))
    model.eval()

    for target, mask, fn in dataset:
        print(f"[**] inpainting {fn} ... ")
        target = target.unsqueeze(0)
        mask = mask.unsqueeze(0)

        with torch.no_grad():
            masked_tensor = (target * (1 - mask).float()) + mask
            pred_tensor = model(masked_tensor, mask)
            comp_tensor = pred_tensor * mask + target * (1 - mask)

            fig, axs = plt.subplots(2, 2, figsize=(12, 12), sharex=True, sharey=False)

            target = target.reshape((256, 256))
            mask = mask.reshape((256, 256))
            masked_tensor = masked_tensor.reshape((256, 256))
            comp_tensor = comp_tensor.reshape((256, 256))

            axs[0, 0].contourf(target.numpy(), levels=100, cmap="terrain")
            axs[0, 1].contourf(mask.numpy(), levels=100, cmap="gray")
            axs[1, 0].contourf(masked_tensor.numpy(), levels=100, cmap="terrain")
            axs[1, 1].contourf(comp_tensor.numpy(), levels=100, cmap="terrain")

            fig.tight_layout()
            plt.show()


if __name__ == "__main__":
    demo(args)
